...
def pairwise_inner_3d(x: Tensor, y: Tensor, curv: float = 1.0, eps: float = 1e-8) -> Tensor:
    """
    Compute pairwise Lorentzian inner product between input vectors in a 3D context.

    Args:
        x: Tensor of shape `(B, N, D)` giving a space components of a batch
            of vectors on the hyperboloid.
        y: Tensor of shape `(B, M, D)` giving a space components of another
            batch of points on the hyperboloid.
        curv: Positive scalar denoting negative hyperboloid curvature.
        eps: Small float number to avoid numerical instability.

    Returns:
        Tensor of shape `(B, N, M)` giving pairwise Lorentzian inner product
        between input vectors.
    """
    # Compute time-like components based on the hyperboloid constraint
    x_time = torch.sqrt(torch.clamp(1 / curv + torch.sum(x**2, dim=-1, keepdim=True), min=eps))
    y_time = torch.sqrt(torch.clamp(1 / curv + torch.sum(y**2, dim=-1, keepdim=True), min=eps))

    # Concatenate the time components to form full Lorentz coordinates
    x_lorentz = torch.cat([x_time, x], dim=-1)  # Shape: (B, N, D+1)
    y_lorentz = torch.cat([y_time, y], dim=-1)  # Shape: (B, M, D+1)

    # Compute the Lorentzian inner product
    # x_lorentz: (B, N, D+1)
    # y_lorentz: (B, M, D+1)
    # Resulting shape will be (B, N, M)
    time_product = -x_lorentz[..., 0:1] @ y_lorentz[..., 0:1].transpose(1, 2)  # Time component dot product
    space_product = torch.matmul(x_lorentz[..., 1:], y_lorentz[..., 1:].transpose(1, 2))  # Space components dot product
    
    # Combine the time and space products
    xyl = time_product + space_product  # Shape: (B, N, M)
...
def pairwise_dist_3d(
    ...
    Args:
        x: Tensor of shape `(B1, NP, D)` giving a space components of a batch
            of point on the hyperboloid.
        y: Tensor of shape `(B2, NP, D)` giving a space components of another
            batch of points on the hyperboloid.
        curv: Positive scalar denoting negative hyperboloid curvature.
        eps: Small float number to avoid numerical instability.

    Returns:
        Tensor of shape `(B1, NP, B2, NP)` giving pairwise distance along the geodesics
        connecting the input points.
    """
    # Ensure numerical stability in arc-cosh by clamping input.
